...
class INode {
    public:
        ...
        virtual IEdge* getDDProduct(long n, IUniqueTable* ut) = 0;
        virtual IEdge* getDDProductParallel(long n, IUniqueTable* ut, int level) = 0;
        virtual IEdge* getDDProductParallelCached(long n, IUniqueTable* ut, int level) = 0;
        virtual IEdge* getDDProductParallelPrivate(long n, IUniqueTable* ut, int level) = 0;
};
...
class IEdge {
    public:
        ...
        virtual IEdge* getDDProduct(IUniqueTable* ut) = 0;
        virtual IEdge* getDDProductParallel(IUniqueTable* ut) = 0;
        virtual IEdge* getDDProductParallelCached(IUniqueTable* ut) = 0;
        virtual IEdge* getDDProductParallel(IUniqueTable* ut, int level) = 0;
        virtual IEdge* getDDProductParallelCached(IUniqueTable* ut, int level) = 0;
        virtual IEdge* getDDProductParallelPrivate(IUniqueTable* ut, int level) = 0;
};
...
class Edge: public IEdge {
    ...
    public:
        ...
        IEdge* getDDProduct(IUniqueTable* ut) {
            return node->getDDProduct(n, ut);
        }

        IEdge* getDDProductParallel(IUniqueTable* ut) {
            return getDDProductParallel(ut, 1);
        }

        IEdge* getDDProductParallel(IUniqueTable* ut, int level) {
            return node->getDDProductParallel(n, ut, level);
        }

        IEdge* getDDProductParallelCached(IUniqueTable* ut) {
            return getDDProductParallelCached(ut, 1);
        }

        IEdge* getDDProductParallelCached(IUniqueTable* ut, int level) {
            return node->getDDProductParallelCached(n, ut, level);
        }

        IEdge* getDDProductParallelPrivate(IUniqueTable* ut, int level) {
            return node->getDDProductParallelPrivate(n, ut, level);
        }
...
class Node : public INode {
    ...
    public:
        ...
        IEdge* getDDProductParallel(long n, IUniqueTable* ut, int level) {
            long value = n;
            long leftValue = 1;
            long rightValue = 1;
            IEdge* leftEdge = nullptr;
            IEdge* rightEdge = nullptr;
            if (level == 0)
                return this->getDDProduct(n, ut);
            #pragma omp task shared(leftValue, rightValue, leftEdge, rightEdge, level)
            {
                if (this->leftEdge != nullptr) {
                    leftEdge = this->leftEdge->getDDProductParallel(ut, level-1);
            ...
            {
                if (this->rightEdge != nullptr) {
                    rightEdge = this->rightEdge->getDDProductParallel(ut, level-1);
        ...
        IEdge* getDDProductParallelCached(long n, IUniqueTable* ut, int level) {
            long value = n;
            long leftValue = 1;
            long rightValue = 1;
            IEdge* leftEdge = nullptr;
            IEdge* rightEdge = nullptr;
            if (level == 0)
                return this->getDDProduct(n, new CachedUniqueTable(ut));
            #pragma omp task shared(leftValue, rightValue, leftEdge, rightEdge, level)
            {
                if (this->leftEdge != nullptr) {
                    leftEdge = this->leftEdge->getDDProductParallelCached(ut, level-1);
                    leftValue = leftEdge->getValue();
                }
            }
            #pragma omp task shared(leftValue, rightValue, leftEdge, rightEdge, level)
            {
                if (this->rightEdge != nullptr) {
                    rightEdge = this->rightEdge->getDDProductParallelCached(ut, level-1);
                    rightValue = rightEdge->getValue();
                }
            }
            #pragma omp taskwait
        ...
        IEdge* getDDProductParallelPrivate(long n, IUniqueTable* ut, int level) {
            long value = n;
            long leftValue = 1;
            long rightValue = 1;
            IEdge* leftEdge = nullptr;
            IEdge* rightEdge = nullptr;
            if (level == 0)
                return this->getDDProduct(n, new UniqueTablePrivate());
            #pragma omp task shared(leftValue, rightValue, leftEdge, rightEdge, level)
            {
                if (this->leftEdge != nullptr) {
                    leftEdge = this->leftEdge->getDDProductParallelPrivate(ut, level-1);
                    leftValue = leftEdge->getValue();
                }
            }
            #pragma omp task shared(leftValue, rightValue, leftEdge, rightEdge, level)
            {
                if (this->rightEdge != nullptr) {
                    rightEdge = this->rightEdge->getDDProductParallelPrivate(ut, level-1);
                    rightValue = rightEdge->getValue();
                }
            }
            #pragma omp taskwait
...
class DD : public IDD {
    ...
    public:
        ...
        long getDDProduct() {
            headEdge = headEdge->getDDProduct(ut);
        ...
        long getDDProductParallel(int level) {
            #pragma omp parallel num_threads(level*2+1) shared(headEdge)
            {
                ...
                {
                    #pragma omp task
                    headEdge = headEdge->getDDProductParallel(ut, level);
                    #pragma omp taskwait
                }
            }
            return headEdge->getValue();
        ...
        long getDDProductParallelCached(int level) {
            #pragma omp parallel num_threads(level*2+1) shared(headEdge)
            {
                ...
                {
                    #pragma omp task
                    headEdge = headEdge->getDDProductParallelCached(ut, level);
                    #pragma omp taskwait
                }
            }
            return headEdge->getValue();
        ...
        long getDDProductParallelPrivate(int level) {
            #pragma omp parallel num_threads(level*2+1) shared(headEdge)
            {
                ...
                {
                    #pragma omp task
                    headEdge = headEdge->getDDProductParallelPrivate(ut, level);
                    #pragma omp taskwait
                }
            }
            return headEdge->getValue();
...
DD* createLargeDD() {
    ...
    Edge* rightNode1 = new Edge(2, new Node(nodeArray[0], nodeArray[0]));
    Edge*  leftNode2 = new Edge(2, new Node(nodeArray[0], nodeArray[0]));
    Edge* rightNode2 = new Edge(2, new Node(nodeArray[1], nodeArray[1]));
    Edge*  leftNode3 = new Edge(2, new Node(nodeArray[1], nodeArray[0]));
    Edge* rightNode3 = new Edge(2, new Node(nodeArray[0], nodeArray[1]));
    Edge*  leftNode4 = new Edge(2, new Node(nodeArray[0], nodeArray[1]));
    Edge* rightNode4 = new Edge(2, new Node(nodeArray[1], nodeArray[0]));

    leftNode1  = new Edge(3, new Node(leftNode1, rightNode1));
    rightNode1 = new Edge(3, new Node(leftNode2, rightNode2));
    leftNode2  = new Edge(3, new Node(leftNode3, rightNode3));
    rightNode2 = new Edge(3, new Node(leftNode4, rightNode4));
