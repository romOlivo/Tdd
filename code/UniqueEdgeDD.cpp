...
class INode {
    public:
        ...
        virtual IEdge* getDDProductParallel(long n, IUniqueTable* ut) = 0;
        virtual IEdge* getDDProductParallelCached(long n, IUniqueTable* ut) = 0;
};
...
class Edge: public IEdge {
    ...
    public:
        ...
        IEdge* getDDProductParallel(IUniqueTable* ut) {
            return node->getDDProductParallel(n, ut);
        }

        IEdge* getDDProductParallelCached(IUniqueTable* ut) {
            return node->getDDProductParallelCached(n, ut);
        }
...
class Node : public INode {
    ...
    public:
        ...
        IEdge* getDDProductParallel(long n, IUniqueTable* ut) {
            long value = n;
            long leftValue = 1;
            long rightValue = 1;
            IEdge* leftEdge = nullptr;
            IEdge* rightEdge = nullptr;
            #pragma omp parallel num_threads(2) shared(leftValue, rightValue, leftEdge, rightEdge)
            {
                #pragma omp single 
                {
                    #pragma omp task
        ...
        IEdge* getDDProductParallelCached(long n, IUniqueTable* ut) {
            long value = n;
            long leftValue = 1;
            long rightValue = 1;
            IEdge* leftEdge = nullptr;
            ...
            #pragma omp parallel num_threads(2) shared(leftValue, rightValue, leftEdge, rightEdge)
            {
                #pragma omp single 
                {
                    ...
                    {
                        if (this->leftEdge != nullptr) {
                            leftEdge  = this->leftEdge->getDDProduct(new CachedUniqueTable(ut));
                            leftValue = leftEdge->getValue();
                        }
                    }
                    #pragma omp task
                    {
                        if (this->rightEdge != nullptr) {
                            rightEdge = this->rightEdge->getDDProduct(new CachedUniqueTable(ut));
                            rightValue = rightEdge->getValue();
                        }
                    }
                }
...
class DD : public IDD {
    ...
    public:
        ...
        long getDDProductParallel() {
            headEdge = headEdge->getDDProductParallel(ut);
            return headEdge->getValue();
        }
        long getDDProductParallelCached() {
            headEdge = headEdge->getDDProductParallelCached(ut);
            return headEdge->getValue();
        }
    private:
        IEdge* headEdge;
...
void printParallelRuns(DD* dd, int numIters) {
    print("  # Parallel run:");
    for(int i = 1; i <= numIters; i++) {
        auto start = chrono::high_resolution_clock::now();
        auto res = dd->getDDProductParallel();
        chrono::duration<double, std::milli> duration = chrono::high_resolution_clock::now() - start;
        cout << "   --> Iter: " << i << "\t time: " << duration.count() << "\t result: " << res << "\n";
    }
}

void printParallelCachedRuns(DD* dd, int numIters) {
    print("  # Cached parallel run:");
    for(int i = 1; i <= numIters; i++) {
        auto start = chrono::high_resolution_clock::now();
        auto res = dd->getDDProductParallelCached();
        chrono::duration<double, std::milli> duration = chrono::high_resolution_clock::now() - start;
        cout << "   --> Iter: " << i << "\t time: " << duration.count() << "\t result: " << res << "\n";
    }
}
...
int main() {
    ...
    printParallelRuns(ddSmallParallel, TIMES);
    print(" Small DD product tested.\n");

    print(" Testing large DD product...");
    printSequentialRuns(ddLargeSequential, TIMES);
    printParallelRuns(ddLargeParallel, TIMES);
    printParallelCachedRuns(ddLargeParallelCached, TIMES);
    print(" Large DD product tested.\n");

    print(" Testing equal DD product...");
    printSequentialRuns(ddEqualSequential, TIMES);
    printParallelRuns(ddEqualParallel, TIMES);
    printParallelCachedRuns(ddEqualParallelCached, TIMES);
